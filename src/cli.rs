...
mod tests {
    ...
    #[test]
    fn parses_ssh_setup_and_proxy_commands() {
        let setup = Cli::try_parse_from(["awsome", "setup-ssh"]).unwrap();
        assert!(matches!(setup.command, Some(Command::SetupSsh)));

        let proxy = Cli::try_parse_from(["awsome", "ssh-proxy", "2222"]).unwrap();
        assert!(matches!(
            proxy.command,
            Some(Command::SshProxy { port: 2222 })
        ));
    }

    #[test]
    fn parses_default_start_and_shutdown_schedule() {
        let default_command = Cli::try_parse_from(["awsome"]).unwrap();
        assert!(default_command.command.is_none());

        let scheduled = Cli::try_parse_from(["awsome", "start", "--schedule-shutdown"]).unwrap();
        assert!(matches!(
            scheduled.command,
            Some(Command::Start {
                schedule_shutdown: Some(ref time)
            }) if time == "18:30"
        ));
    }

    #[test]
    fn requires_profile_and_instance_id_together() {
        assert!(Cli::try_parse_from(["awsome", "configure", "add", "--profile", "dev"]).is_err());
        assert!(
            Cli::try_parse_from([
                "awsome",
                "configure",
    ...
    #[test]
    fn parses_non_interactive_group_configuration() {
        ...
        assert!(matches!(
            ...
            Some(Command::Configure {
                action: ConfigureAction::Add {
                    ...
                    instance_id: Some(ref instance_id),
                }
            }) if profile == "development" && instance_id == "i-0123456789abcdef0"
        ));
    }
}
